...
class Average(AggregationMethod):
    """
    Calculate average value of a set of numbers.
    """

    def __init__(self, id="average", label="average", group="aggregate"):
        super().__init__(id, label, group)

    def get_header_display_value(self, oConcept, td_entry):
        return "{} (average)".format(oConcept.name)
    ...
    def calculate_agg_value(self, td_entry, value, display_processor_sort_value_func=None):
        ...
        if not value:
            return None
        return helpers.round_sig_3(mean(value), round_whole_number=False)
