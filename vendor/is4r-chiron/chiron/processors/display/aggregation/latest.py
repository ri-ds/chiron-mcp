...
class Latest(AggregationMethod):
    """
    Get the latest value (most recent) based on the event_date_field for the collection.
    """

    def __init__(self, id="latest", label="latest value", group="aggregate"):
        super().__init__(id, label, group)

    def get_header_display_value(self, oConcept, td_entry):
        position = td_entry.get("aggregation_settings", {}).get("latest_ordinal_position", 1)
        if position == 1:
            return "{} (latest)".format(oConcept.name)
        return "{} ({} latest)".format(oConcept.name, self._get_ordinal_string(position))
    ...
    def get_inputs(self, chironuser, td_entry):
        options = [(1, "latest value (default)")]
        for n in range(2, 21):
            ord = self._get_ordinal_string(n)
            options.append((n, "{} latest value".format(ord)))
        inputs = [
            {
                "id": "latest_ordinal_position",
                "label": "select specific ordinal position",
                "type": "select",
                "selected": td_entry.get("aggregation_settings", {}).get(
                    "latest_ordinal_position", 1
                ),
                "options": options,
            }
        ]
    ...
    def set_custom_settings(self, input_data):
        position = int(input_data.get("latest_ordinal_position", "1"))
        return {
            "latest_ordinal_position": position,
        }
    ...
    def calculate_agg_value(self, td_entry, value, display_processor_sort_value_func=None):
        ...
        value = sorted(value, key=lambda k: k[0], reverse=True)
