...
from chiron.api_v2.serializers import ConceptSerializer
...
class ConceptViewSetV2(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # get dataset name
        dataset_name = self.kwargs.get("dataset_string", None)
        # Get initial queryset
        queryset = models.Concept.objects.filter(collection__dataset__unique_id=dataset_name)
        ...
        queryset = queryset.filter(id__in=allowed_concept_ids)
    ...
    def retrieve(self, request, permanent_id=None, *args, **kwargs):
        ...
        :param cohort_def: the current cohort definition for calculating statistics, if None
          then the active cohort definition will be used
        ...
        :return: A serialized concept; plus optionally statistics, cohort_def_options, and
          callback_data
        """
        oConcept = get_object_or_404(
            models.Concept.objects.filter(collection__dataset__unique_id=kwargs["dataset_string"]),
            permanent_id=permanent_id,
        )
        serializer = self.serializer_class(oConcept)
        ...
        if string_to_bool(request.query_params.get("include_cohort_def_options")):
            data["cohort_def_options"] = self._get_cohort_def_options(
                request, permanent_id, prefilter_value, *args, **kwargs
            )
        if string_to_bool(request.query_params.get("include_table_def_options")):
            data["table_def_options"] = self._get_table_def_options(
                request, permanent_id, *args, **kwargs
            )
        ...
        if data["concept_for_prefilter"] and string_to_bool(
            ...
            data["concept_for_prefilter"]["cohort_def_options"] = self._get_cohort_def_options(
                request, data["concept_for_prefilter"]["permanent_id"], *args, **kwargs
            )
        data["prefilter_value"] = prefilter_value
        return Response(data)

    def update(self, request, permanent_id=None, *args, **kwargs):
        """
    ...
    def _get_concept_statistics(self, request, permanent_id, *args, **kwargs):
        oConcept = get_object_or_404(
            models.Concept.objects.filter(
                collection__dataset__unique_id=self.kwargs["dataset_string"]
            ),
            permanent_id=permanent_id,
        )
        ...
        # if no cohort_def provided in request, use the active snapshot
    ...
    def _get_table_def_options(self, request, permanent_id, *args, **kwargs):
        # get all of the data from the request
        params = request.query_params
        existing_entry_id = params.get("table_def_entry_id")
        # if no table_def provided in request, use the active snapshot
        if "table_def" in params:
            table_def = convert_to_obj(params["table_def"])
        else:
    ...
    def _get_cohort_def_options(
        self, request, permanent_id, prefilter_value=None, *args, **kwargs
    ):
        # get all of the data from the request
        params = request.query_params
        ...
        if existing_entry_id:
            response["entry_id"] = existing_entry_id
            # current special case for adding term labels for ontology display
        return response
    ...
    def _get_concept_callback_data(self, request, permanent_id):
        ...
        if string_to_bool(params.get("show_all_data")):
            ...
            cohort_def_for_display = cohort_def
        oConcept = get_object_or_404(
            models.Concept.objects.filter(collection__dataset=request.dataset),
            permanent_id=permanent_id,
        )
