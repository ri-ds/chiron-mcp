...
class PgAggCountDistinct(AbstractPostgresAggregator):
    def create_agg_statement(
        ...
        if double_aggregation:
            return statement
        return func.json_arr_to_count_distinct(statement)

    def create_outer_agg_statement(self, subquery, table, column_alias=None):
        response = func.json_arr_set_to_count_distinct(subquery)
        if column_alias:
