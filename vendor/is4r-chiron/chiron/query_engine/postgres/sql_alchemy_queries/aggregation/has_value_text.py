...
class PgAggHasValueText(AbstractPostgresAggregator):
    def __init__(self, chironuser, td_entry):
        self.selected_values = td_entry.get("aggregation_settings", {}).get("values_text", [])
    ...
    def _get_agg_function(self):
        if self.return_value in ["boolean"]:
    ...
    def create_agg_statement(
        ...
        elif self.return_value in ["all_values", "earliest_value", "latest_value"]:
            if self.return_value == "all_values":
                response = func.get_matching_values_in_json_arr_alphabetical(
                    statement, self.values_to_postgres_array_string()
                )
            if self.return_value == "earliest_value":
                if chiron_settings.CHIRON_EVENT_CONCEPT_TYPE == "detailed_age":
                    response = func.get_matching_values_in_json_arr_by_age(
                        statement, self.values_to_postgres_array_string()
                    )
                else:
                    response = func.get_matching_values_in_json_arr_chron(
                        statement, self.values_to_postgres_array_string()
                    )
                response = func.get_array_position(response, 1)
            if self.return_value == "latest_value":
                if chiron_settings.CHIRON_EVENT_CONCEPT_TYPE == "detailed_age":
                    response = func.get_matching_values_in_json_arr_by_age(
                        statement, self.values_to_postgres_array_string()
                    )
