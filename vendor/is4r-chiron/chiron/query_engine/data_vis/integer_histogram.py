...
class IntegerHistogram:
    ...
    def __init__(self, value_count, min_val, max_val, number_is_year=False):
        ...
        if self.bin_count > (self.histogram_max_val - self.histogram_min_val):
            self.bin_count = self.histogram_max_val - self.histogram_min_val
    ...
    def get_bin_cutoffs(self):
        ...
        There will be one more bin cutoff than there are bins. For integers the calculated bin
        size will be rounded to an integer, which may change the number of bins.
        """
    ...
    def make_bin_label(self, min_cutoff, min_inclusive, max_cutoff, max_inclusive):
        if self.number_is_year:
            return self.make_bin_label_for_years(
                min_cutoff, min_inclusive, max_cutoff, max_inclusive
            )
    ...
    def number_to_string(self, value):
        ...
        if self.use_k:
            if value == 0:
                return "0"
            if value >= 100_000:
                return "{:.0f}".format(value / 1000).rstrip("0") + "k"
            if value >= 10_000:
                return "{:.1f}".format(value / 1000).rstrip("0") + "k"
            else:
                return "{:.2f}".format(value / 1000).rstrip("0") + "k"
