...
class IntervalDateRangeOption(AbstractEventOption):
    fields = ["date_start_min", "date_start_max", "date_end_min", "date_end_max"]

    def is_deidentified(self):
        return False

    def get_unique_name(self):
        return "interval_date_range"
    ...
    def get_option_data(self):
        ...
        if rule.get("type") == "interval_date_range":
            data["date_start_min"] = rule.get("date_start_min", "")
            data["date_start_max"] = rule.get("date_start_max", "")
            data["date_end_min"] = rule.get("date_end_min", "")
            data["date_end_max"] = rule.get("date_end_max", "")
        else:
            data["date_start_min"] = ""
            data["date_start_max"] = ""
            data["date_end_min"] = ""
    ...
    def get_form_template(self):
        return "chiron/event_date_forms/interval_date_range.html"
    ...
    def event_rule_to_string(self):
        ...
        if has_min and has_max:
            response.append(
                "{} <= {} start date <= {}".format(
                    self.existing_rule["date_start_min"],
                    self.collection.name,
                    self.existing_rule["date_start_max"],
                )
            )
        elif has_min:
            response.append(
        ...
        if has_min and has_max:
            response.append(
                "{} <= {} end date <= {}".format(
                    self.existing_rule["date_end_min"],
                    self.collection.name,
                    self.existing_rule["date_end_max"],
                )
            )
        elif has_min:
            response.append(
